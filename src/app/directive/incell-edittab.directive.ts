...
@Directive({
    selector: '[inCellEditTab]'
})
export class InCellEditTabDirective implements OnInit, OnDestroy {
    @Input('inCellEditTab')
    public createFormGroup: (dataItem:any, dataPoints:any[],columns:any[], parentIndex:number) => FormGroup;
    ...
    public onKeydown(e: KeyboardEvent): void {
        ...
        if (dataItem) {
            // Edit focused cell
            const formGroup = this.createFormGroup(null,activeRow.dataItem,this.grid.columns.toArray(),0);
